...
*Setup instructions for a gitian build of solidreserve using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the solidreserve
...
- [Building solidreserve](#building-solidreserve)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-solidreserve-host/README.md).
...
Clone the git repositories for solidreserve and gitian and then checkout the solidreserve version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/solidreserve-crypto/solidreserve.git
cd solidreserve
...
Follow the instructions in [doc/release-process.md](release-process.md) in the solidreserve repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building solidreserve
----------------

To build solidreserve (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the solidreserve repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/solidreserve/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/solidreserve-crypto/solidreserve
...
For example:
```bash
URL=https://github.com/crowning-/solidreserve.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit solidreserve=${COMMIT} --url solidreserve=${URL} ../solidreserve/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit solidreserve=${COMMIT} --url solidreserve=${URL} ../solidreserve/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit solidreserve=${COMMIT} --url solidreserve=${URL} ../solidreserve/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/solidreserve-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/solidreserve-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/solidreserve-build.assert
```
...
[solidreserve/gitian.sigs](https://github.com/solidreserve-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
