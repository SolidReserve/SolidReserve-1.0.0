...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total solidreserve balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized solidreserve balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in solidreserve/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in solidreserve/kb\n"
    ...
    Object operator()(const CScriptID& scriptID) const
    {
        ...
        if (mine != ISMINE_NO) {
            ...
            BOOST_FOREACH (const CTxDestination& addr, addresses)
                a.push_back(CSolidreserveAddress(addr).ToString());
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"solidreserveaddress\"\n"
            "\nReturn information about the given solidreserve address.\n"
            "\nArguments:\n"
            "1. \"solidreserveaddress\"     (string, required) The solidreserve address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"solidreserveaddress\", (string) The solidreserve address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: solidreserve address and we have full public key:
        CSolidreserveAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are solidreserve addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) solidreserve address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
    ...
    CSolidreserveAddress address(innerID);
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"solidreserveaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"solidreserveaddress\"  (string, required) The solidreserve address to use for the signature.\n"
