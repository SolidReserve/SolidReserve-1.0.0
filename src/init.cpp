...
void PrepareShutdown()
{
    ...
    RenameThread("solidreserve-shutoff");
    mempool.AddTransactionsUpdated(1);
    StopRPCThreads();
#ifdef ENABLE_WALLET
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateSolidreserves(false, NULL, 0);
#endif
    StopNode();
    DumpMasternodes();
    DumpBudgets();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "solidreserve.conf"));
    if (mode == HMM_SOLIDRESERVED) {
#if !defined(WIN32)
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "solidreserved.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, solidreserve, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_SOLIDRESERVE_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all solidreserve specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizesolidreserveamount=<n>", strprintf(_("Keep N PIE anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageGroup(_("RPC SSL options: (see the Solidreserve Wiki for SSL setup instructions)"));
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Solidreserve Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Dash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The solidreserve Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("solidreserve-loadblk");
...
/** Sanity checks
 *  Ensure that solidreserve is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if (!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.solidreserve.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. solidreserve Core is shutting down."));
...
#endif
    // Make sure only a single solidreserve process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. solidreserve Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("solidreserve version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of solidreserve Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart solidreserve Core to complete") << "\n";
    ...
    nAnonymizesolidreserveAmount = GetArg("-anonymizesolidreserveamount", 0);
    if (nAnonymizesolidreserveAmount > 999999) nAnonymizesolidreserveAmount = 999999;
    if (nAnonymizesolidreserveAmount < 2) nAnonymizesolidreserveAmount = 2;
    ...
    LogPrintf("Anonymize solidreserve Amount %d\n", nAnonymizesolidreserveAmount);
    ...
    */
    ...
    if (pwalletMain)
        GenerateSolidreserves(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", 1));
#endif
