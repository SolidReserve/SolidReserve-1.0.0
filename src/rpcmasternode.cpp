...
// Copyright (c) 2009-2012 The Solidreserve developers
...
Value obfuscation(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <solidreserveaddress> <amount>\n"
            "solidreserveaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <solidreserveaddress> <amount>\n"
            "solidreserveaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in solidreserve.conf\n"
    ...
    if (strCommand == "current") {
        ...
        if (winner) {
            ...
            obj.push_back(Pair("pubkey", CSolidreserveAddress(winner->pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "genkey") {
        ...
        return CSolidreserveSecret(secret).ToString();
    }
    ...
    if (strCommand == "status") {
        ...
        if (pmn) {
            ...
            mnObj.push_back(Pair("addr", CSolidreserveAddress(pmn->pubKeyCollateralAddress.GetID()).ToString()));
...
Value masternodelist(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH (PAIRTYPE(int, CMasternode) & s, vMasternodeRanks) {
        ...
        if (strFilter != "" && strTxHash.find(strFilter) == string::npos &&
            ...
            CSolidreserveAddress(mn->pubKeyCollateralAddress.GetID()).ToString().find(strFilter) == string::npos) continue;
        ...
        obj.push_back(Pair("addr", CSolidreserveAddress(mn->pubKeyCollateralAddress.GetID()).ToString()));
