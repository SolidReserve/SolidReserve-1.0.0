...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(SolidreserveUnits::PIE)
    {
    }
    ...
    inline void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
    {
        ...
        painter->setPen(foreground);
        QString amountText = SolidreserveUnits::formatWithUnit(unit, amount, true, SolidreserveUnits::separatorAlways);
        if (!confirmed) {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance, false, SolidreserveUnits::separatorAlways));
    ui->labelUnconfirmed->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, unconfirmedBalance, false, SolidreserveUnits::separatorAlways));
    ui->labelImmature->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, immatureBalance, false, SolidreserveUnits::separatorAlways));
    ui->labelAnonymized->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, anonymizedBalance, false, SolidreserveUnits::separatorAlways));
    ui->labelTotal->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, balance + unconfirmedBalance, false, SolidreserveUnits::separatorAlways));


    ui->labelWatchAvailable->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, watchOnlyBalance, false, SolidreserveUnits::separatorAlways));
    ui->labelWatchPending->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, watchUnconfBalance, false, SolidreserveUnits::separatorAlways));
    ui->labelWatchImmature->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, watchImmatureBalance, false, SolidreserveUnits::separatorAlways));
    ui->labelWatchTotal->setText(SolidreserveUnits::floorHtmlWithUnit(nDisplayUnit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, SolidreserveUnits::separatorAlways));
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizesolidreserveAmount = SolidreserveUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizesolidreserveAmount * COIN, false, SolidreserveUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizesolidreserveAmount = strAnonymizesolidreserveAmount.remove(strAnonymizesolidreserveAmount.indexOf("."), SolidreserveUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizesolidreserveAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizesolidreserveAmount * COIN) nMaxToAnonymize = nAnonymizesolidreserveAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizesolidreserveAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizesolidreserveAmount));
        strAnonymizesolidreserveAmount = strAnonymizesolidreserveAmount.remove(strAnonymizesolidreserveAmount.indexOf("."), SolidreserveUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizesolidreserveAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
        QString strMaxToAnonymize = SolidreserveUnits::formatHtmlWithUnit(nDisplayUnit, nMaxToAnonymize, false, SolidreserveUnits::separatorAlways);
        ui->labelAmountRounds->setToolTip(tr("Not enough compatible inputs to anonymize <span style='color:red;'>%1</span>,<br>"
                                             "will anonymize <span style='color:red;'>%2</span> instead")
                                              .arg(strAnonymizesolidreserveAmount)
                                              .arg(strMaxToAnonymize));
        strMaxToAnonymize = strMaxToAnonymize.remove(strMaxToAnonymize.indexOf("."), SolidreserveUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = "<span style='color:red;'>" +
                             QString(SolidreserveUnits::factor(nDisplayUnit) == 1 ? "" : "~") + strMaxToAnonymize +
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (balance < minAmount) {
            QString strMinAmount(SolidreserveUnits::formatWithUnit(nDisplayUnit, minAmount));
            QMessageBox::warning(this, tr("Obfuscation"),
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizesolidreserveAmount == 0) {
