    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return SolidreserveUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, SolidreserveUnits::separatorNever);
            else
                return SolidreserveUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    if (this->walletModel->getOptionsModel() != NULL) {
        amountTitle += " (" + SolidreserveUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")";
    }
    return amountTitle;
}
