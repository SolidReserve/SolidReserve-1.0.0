...
#include "solidreserveaddressvalidator.h"
...
#define URI_SCHEME "solidreserve"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setFont(solidreserveAddressFont());
#if QT_VERSION >= 0x040700
    // We don't want translators to use own addresses in translations
    // and this is the only place, where this address is supplied.
    widget->setPlaceholderText(QObject::tr("Enter a solidreserve address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
    widget->setValidator(new SolidreserveAddressEntryValidator(parent));
    widget->setCheckValidator(new SolidreserveAddressCheckValidator(parent));
}
...
bool parseSolidreserveURI(const QUrl& uri, SendCoinsRecipient* out)
{
    // return if URI is not valid or is no solidreserve: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!SolidreserveUnits::parse(SolidreserveUnits::PIE, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseSolidreserveURI(QString uri, SendCoinsRecipient* out)
{
    // Convert solidreserve:// to solidreserve:
    //
    //    Cannot handle this later, because solidreserve:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if (uri.startsWith(URI_SCHEME "://", Qt::CaseInsensitive)) {
        uri.replace(0, std::strlen(URI_SCHEME) + 3, URI_SCHEME ":");
    }
    QUrl uriInstance(uri);
    return parseSolidreserveURI(uriInstance, out);
}

QString formatSolidreserveURI(const SendCoinsRecipient& info)
{
    QString ret = QString(URI_SCHEME ":%1").arg(info.address);
    int paramCount = 0;

    if (info.amount) {
        ret += QString("?amount=%1").arg(SolidreserveUnits::format(SolidreserveUnits::PIE, info.amount, false, SolidreserveUnits::separatorNever));
        paramCount++;
    }
...
{
    CTxDestination dest = CSolidreserveAddress(address.toStdString()).Get();
...
void openConfigfile()
{
    ...
    /* Open solidreserve.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "solidreserve.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for solidreserve.lnk
...
{
    return GetAutostartDir() / "solidreserve.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a solidreserve.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=solidreserve\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the solidreserve app
...
bool GetStartOnSystemStartup()
{
    CFURLRef solidreserveAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    LSSharedFileListRef loginItems = LSSharedFileListCreate(NULL, kLSSharedFileListSessionLoginItems, NULL);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, solidreserveAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add solidreserve app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, solidreserveAppUrl, NULL, NULL);
