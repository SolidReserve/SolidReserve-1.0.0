...
{
    CSolidreserveAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered solidreserve address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CSolidreserveAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal solidreserve:URI (solidreserve:XyZ...?message=example)
            {
...
{
    QString strAddress = QString::fromStdString(CSolidreserveAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CSolidreserveAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CSolidreserveAddress(sAddress).Get();
...
bool WalletModel::isMine(CSolidreserveAddress address)
{
