...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = SolidreserveUnits::formatWithUnit(SolidreserveUnits::PIE, nValue);
...
std::string AddressToString(const CSolidreserveAddress& Address)
{
    std::string TxLabels[] =
        {
            _("Date"),
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (solidreserve.conf).");
            QMessageBox::warning(this, "solidreserve Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
