...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or solidreserve.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    } else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateSolidreserves(fGenerate, pwalletMain, nGenProcLimit);
    }
...
// NOTE: Unlike wallet RPC (which use OLID values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.solidreserve.it/wiki/BIP_0022 for full specification.\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "solidreserve is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "solidreserve is downloading blocks...");
