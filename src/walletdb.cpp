...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name") {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CSolidreserveAddress(strAddress).Get()].name;
        } else if (strType == "purpose") {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CSolidreserveAddress(strAddress).Get()].purpose;
        } else if (strType == "tx") {
            uint256 hash;
            ssKey >> hash;
            CWalletTx wtx;
        ...
        } else if (strType == "multisend") //presstab HyperStake
        {
            ...
            std::pair<std::string, int> pMultiSend;
            ssValue >> pMultiSend;
            if (CSolidreserveAddress(pMultiSend.first).IsValid()) {
        ...
        } else if (strType == "mdisabled") //presstab HyperStake
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CSolidreserveAddress(strAddress).Get(), strKey, strValue)) {
...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("solidreserve-wallet");
