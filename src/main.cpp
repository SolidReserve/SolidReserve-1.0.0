...
#if defined(NDEBUG)
#error "solidreserve cannot be compiled without assertions."
#endif
 ...
 * We are ~100 times smaller then solidreserve now (2015-06-23), set minRelayTxFee only 10 times higher
...
{
    RenameThread("solidreserve-scriptch");
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && fFetch /*&& !fImporting*/ && !fReindex) {
            ...
            if (nSyncStarted == 0 || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in Solidreserve
