 ...
 * We are ~100 times smaller then solidreserve now (2015-06-23), set minTxFee 10 times higher
 * so it's still 10 times lower comparing to solidreserve.
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
        std::string strAddr = CSolidreserveAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::GetVinAndKeysFromOutput(COutput out, CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet)
{
    ...
    CSolidreserveAddress address2(address1);
...
map<CSolidreserveAddress, vector<COutput> > CWallet::AvailableCoinsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    vector<COutput> vCoins;
    AvailableCoins(vCoins, fConfirmed);

    map<CSolidreserveAddress, vector<COutput> > mapCoins;
    BOOST_FOREACH (COutput out, vCoins) {
        ...
        mapCoins[CSolidreserveAddress(address)].push_back(out);
    }
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-solidreserve-address
                    CScript scriptChange;
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CSolidreserveAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CSolidreserveAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if (fFileBacked) {
            ...
            std::string strAddress = CSolidreserveAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CSolidreserveAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CSolidreserveAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination& dest, const std::string& key, const std::string& value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CSolidreserveAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination& dest, const std::string& key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CSolidreserveAddress(dest).ToString(), key);
}
...
void CWallet::AutoCombineDust()
{
    ...
    map<CSolidreserveAddress, vector<COutput> > mapCoinsByAddress = AvailableCoinsByAddress(true, 0);

    //coins are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (map<CSolidreserveAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (vDisabledAddresses.size() > 0) {
            for (unsigned int i = 0; i < vDisabledAddresses.size(); i++) {
                if (vDisabledAddresses[i] == CSolidreserveAddress(destMyAddress).ToString()) {
        ...
        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            ...
            CSolidreserveAddress strAddSend(vMultiSend[i].first);
