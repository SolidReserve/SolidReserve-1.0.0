...
# Get BUILDDIR and REAL_SOLIDRESERVED
. "${CURDIR}/tests-config.sh"

export SOLIDRESERVECLI=${BUILDDIR}/qa/pull-tester/run-solidreserve-cli
export SOLIDRESERVED=${REAL_SOLIDRESERVED}
...
if [ "x${ENABLE_SOLIDRESERVED}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and solidreserved must all be enabled"
fi
