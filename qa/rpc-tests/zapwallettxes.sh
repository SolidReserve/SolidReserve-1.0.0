...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars SOLIDRESERVED and SOLIDRESERVECLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

SOLIDRESERVED=${SOLIDRESERVED:-${1}/solidreserved}
CLI=${SOLIDRESERVECLI:-${1}/solidreserve-cli}
...
B1ARGS="-datadir=$D1"
$SOLIDRESERVED $B1ARGS &
B1PID=$!

D2=${D}/node2
CreateDataDir "$D2" port=11010 rpcport=11011
B2ARGS="-datadir=$D2"
$SOLIDRESERVED $B2ARGS &
B2PID=$!
...
$SOLIDRESERVED -zapwallettxes=1 $B1ARGS &
B1PID=$!
$SOLIDRESERVED -zapwallettxes=2 $B2ARGS &
B2PID=$!
